...
/** The desktop yearbook: the big class photos, zoomable, with names on the faces. */
  ...
  return (
    <div className="absolute inset-0">
      ...
      {unknownTag ? (
        ...
        <PersonPanel key={person.id} person={person} onClose={() => go({})} onJump={(s) => go({ personId: person.id, scene: s })} layout="side" />
      ) : null}
    </div>
  )
}
