...
/** The phone yearbook: everyone as a photo tile, then the faces still waiting for a name. No big class photos to download. */
export default function YearbookGrid() {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-6 p-4 pb-28 sm:p-8 sm:pb-28">
      ...
      {unknownTag ? (
        <UnknownPanel key={unknownTag.id} tag={unknownTag} onClose={() => setUnknownTag(null)} onNamed={(id) => (setUnknownTag(null), navigate(`/p/${id}`))} layout="sheet" />
      ) : person ? (
        <PersonPanel key={person.id} person={person} onClose={() => navigate('/')} layout="sheet" />
      ) : null}
    </div>
  )
}
